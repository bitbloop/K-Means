...
unsigned int renderer::pointcloud::create(const unsigned int program_id, const unsigned int data_num, const float* data_xyz, const float* data_rgb)
{
	...
	make_pointcloud_vbo(data_num, data_xyz, data_rgb, pointcloud_ptr->vbo_id, pointcloud_ptr->indexbo_id);
